...
**Present apps to the user with cover images**. The script downloads each app's
cover to a local path (`coverFile`). Deliver them via your agent's native
mechanism:

- **IDE agents (Claude Code, Cursor, Qoder):** render each cover as a markdown
  image and describe it, e.g.
  `1. **全能图片2.0** — 多功能图片生成\n![cover](/tmp/rh-output/app_covers/cover_xxx.png)`
- **Messaging agents (OpenClaw, Slack/Feishu):** use your `message` tool with
  the `media` field set to the local cover path.

After listing, invite a choice: "想试试哪个？告诉我编号就行！也可以说'下一页'看更多～"

Alternatively, if rendering many images is slow, show just the **first 3 covers**
and list the rest as text.

Rules:
- ALWAYS deliver cover images as files/images — NEVER show cover CDN URLs or raw webappId to the user
...
This is critical — AI app tasks are slow, and users need to know the task has started. Notify FIRST, then execute the script.
...
python3 {baseDir}/scripts/runninghub_app.py --run WEBAPP_ID \
  ...
  --file "39:image=/tmp/rh-output/photo.jpg" \
  -o /tmp/rh-output/app_result_$(date +%s).png
...
Same rules as standard API: deliver the saved local file via your agent's
native mechanism, report cost, suggest next steps.
...
python3 {baseDir}/scripts/runninghub_app.py --run 1877265245566922800 \
  --node "52:prompt=a boy with sunglasses" \
  -o /tmp/rh-output/app_$(date +%s).png
